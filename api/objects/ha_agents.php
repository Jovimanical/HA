...
public $updated_at;
	...
	function search($searchKey){
		...
		// select all query
		$query = "SELECT  t.* FROM ". $this->table_name ." t  WHERE t.id LIKE ? OR t.firstname LIKE ?  OR t.lastname LIKE ?  OR t.username LIKE ?  OR t.email LIKE ?  OR t.country_code LIKE ?  OR t.mobile LIKE ?  OR t.balance LIKE ?  OR t.password LIKE ?  OR t.image LIKE ?  OR t.address LIKE ?  OR t.status LIKE ?  OR t.email_verified LIKE ?  OR t.sms_verified LIKE ?  OR t.verification_code LIKE ?  OR t.verification_code_send_at LIKE ?  OR t.two_factor_status LIKE ?  OR t.two_factor_verified LIKE ?  OR t.roles LIKE ?  OR t.featured LIKE ?  OR t.remember_token LIKE ?  OR t.created_at LIKE ?  OR t.updated_at LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
...
$this->updated_at = $row['updated_at'];
	...
	function create(){
		...
		$query ="INSERT INTO ".$this->table_name." SET firstname=:firstname,lastname=:lastname,username=:username,email=:email,country_code=:country_code,mobile=:mobile,balance=:balance,password=:password,image=:image,address=:address,status=:status,email_verified=:email_verified,sms_verified=:sms_verified,verification_code=:verification_code,verification_code_send_at=:verification_code_send_at,two_factor_status=:two_factor_status,two_factor_verified=:two_factor_verified,roles=:roles,featured=:featured,remember_token=:remember_token,created_at=:created_at,updated_at=:updated_at";
...
$stmt->bindParam(":updated_at", $this->updated_at);
	...
	// update the ha_agents
	function update(){
	 
		// update query
		$query ="UPDATE ".$this->table_name." SET firstname=:firstname,lastname=:lastname,username=:username,email=:email,country_code=:country_code,mobile=:mobile,balance=:balance,password=:password,image=:image,address=:address,status=:status,email_verified=:email_verified,sms_verified=:sms_verified,verification_code=:verification_code,verification_code_send_at=:verification_code_send_at,two_factor_status=:two_factor_status,two_factor_verified=:two_factor_verified,roles=:roles,featured=:featured,remember_token=:remember_token,created_at=:created_at,updated_at=:updated_at WHERE id = :id";
