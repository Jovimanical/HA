...
// get ha_kyc_personal_info id
$data =  (json_decode(file_get_contents("php://input"), true ) === NULL ) ? (object)$_REQUEST : json_decode(file_get_contents("php://input"));

 
// set ha_kyc_personal_info id to be deleted
$ha_kyc_personal_info->id = $data->id;
 
// delete the ha_kyc_personal_info
if($ha_kyc_personal_info->delete()){
 
    // set response code - 200 ok
    http_response_code(200);
 
    // tell the user
	echo json_encode(array("status" => "success", "code" => 1,"message"=> "Ha_Kyc_Personal_Info was deleted","document"=> ""));
    
}
 
// if unable to delete the ha_kyc_personal_info
else{
    ...
    // tell the user
	echo json_encode(array("status" => "error", "code" => 0,"message"=> "Unable to delete ha_kyc_personal_info.","document"=> ""));
}
?>
